...
export function BlogCard({ post, index = 0 }: BlogCardProps) {
  ...
  return (
    ...
    >
      ...
      <p className="text-[var(--muted)] text-sm leading-relaxed line-clamp-3">{post.description}</p>

      <div className="flex flex-wrap gap-1.5">
        {post.tags.slice(0, 4).map((tag) => <TechBadge key={tag} label={tag} />)}
      </div>
